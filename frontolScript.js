...
var INVALID_CODE_TO_START_GENERATE_MESSAGE =
...
var INSUFFICIENT_FUNDS_MESSAGE = 'Недостаточно средств для списания';
...
function copyShelfLifeDiscountsFile() {
    ...
    var matchs = path.match(FTP_REGEXP_VAR);
...
function RxShelfLifeDiscounts(doc) {
    ...
    var getAllBarcodeDiscounts = function getAllBarcodeDiscounts() {
        ...
        if (path) {
            var matchs = path.match(FTP_REGEXP_VAR); // проверка: файл на ftp?
...
/* 
Запрос у пользователя ввода глобального параметра
...
Returns:
    true при успехе, false при отказе ввода
*/
...
/* Удаляет глобальые переменные переданные в массиве

Args:
    - settingsArray: массив со списком параметров
*/
function ClearGlobalParameters(settingsArray) {
    //обнуляем глобальные параметры
    for (var i = 0; i < settingsArray.length; i++) {
        var settingName = settingsArray[i];
        frontol.userValues.remove(settingName);
    }
}
...
Extra: {
    ...
    function cashBoxSaleOperation(doc, operatorName, operatorCode, docNumber, barCode, kkmPos) {
        ...
        for (doc.position.index = 1; doc.position.index < doc.position.count; doc.position.index++) {
            ...
            var discount = 0;
            var markup = 0;
            var sectionCode = doc.position.ECRepartment;
            ...
            for (doc.position.index = 1; doc.position.index <= doc.position.count; doc.position.index++) {
                if (doc.position.storno == 0 && doc.position.price > 0) {
                    if (doc.position.ware.maxDiscount != 0) {
                        ...
                        doc.position.setSpecialPrice(frontol.currentDocument.position.ware.price, 0);
                    }
                }
            }
...
function isPromoKassa() {
    var isPromoParticipant = (getGlobalParam('promoParticipation') == '1');
    var uuid = getClientUUID();
    var isValidUUID = (uuid == "A527335D-BE7A-4D52-AAE0-DAC465A0BB5C" || uuid == "72AF8681-AA9D-4FDB-B41F-D9E6C4025C92");
    var hasShopCode = Number(frontol.userValues.get("promoShopCode")) > 0;
    var isPromoAdded = doc.userValues.get("isPromoAdded") === false  //debug

    // все должно быть истина
    return (isPromoParticipant && isValidUUID && hasShopCode && isPromoAdded);
}
